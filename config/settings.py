...
MIDDLEWARE = [
    ...
    'config.middleware.query_logger.QueryLoggingMiddleware'
]

if DEBUG:
    MIDDLEWARE.append('config.middleware.query_logger.QueryLoggingMiddleware')
...
# JWT
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME': timedelta(days=363) if DEBUG else timedelta(days=2),
...
CACHES = {
    'default': {
        'BACKEND': env('CACHE_BACKEND', cast=str, default='django_redis.cache.RedisCache') if not DEBUG else 'django.core.cache.backends.locmem.LocMemCache',
        'LOCATION': env('CACHE_LOCATION', cast=str, default='redis://127.0.0.1:6379/1') if not DEBUG else 'Shop_Portfolio',
        'OPTIONS': {
